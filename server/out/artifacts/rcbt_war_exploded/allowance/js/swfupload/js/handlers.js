...
function fileQueued(file) {
        $('#upload_file').html('已经选择文件:'+file.name);
        swfu.startUpload();
}
...
function uploadSuccess(file, res) {
        res = eval("("+res+")");
        $('#upload_file').html('成功上传文件:'+file.name);
        var fileName1='';
        for(var keys in res.data)
        {
                fileName1="<div style='margin-top:20px;'><span>"+res.data[keys]+"</span><input style='display:none;' class='additional_material_id' value='"+keys+"'/><button type='button' class='layui-btn layui-btn-danger deleteAccessory' onclick='deleteAccessory(this)'><i class='layui-icon'>&#xe640;</i></button><br/></div>"
        }
